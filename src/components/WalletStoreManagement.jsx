...
const WalletStoreManagement = () => {
    ...
    const handleDeleteStore = async (storeId) => {
        setConfirmModal({
            show: true,
            title: 'Delete Wallet Store',
            message: 'Are you sure you want to delete this wallet store? All associated cards will also be deleted.',
            type: 'danger',
            confirmText: 'Delete',
            onConfirm: async () => {
    ...
    const handleExportCardBackPrintSheet = async () => {
        ...
        try {
            ...
            await downloadWalletCardBackPrintSheetImage(cardsToExport, setExportProgress, widthMm, heightMm, cardDesignExport);
        } catch (err) {
    ...
    const handleExportBatchPrintSheetImage = async (batchId) => {
        ...
        try {
            ...
            const widthMm = boardWidth ? parseFloat(boardWidth) * 10 : 900;
            const heightMm = boardHeight ? parseFloat(boardHeight) * 10 : 600;
            await downloadWalletPrintSheetImage(cardsToExport, setExportProgress, null, widthMm, heightMm, accountIdType, cardDesignExport, qrLogo);
        } catch (err) {
                                                    ...
                                                    <td style={{ padding: 'var(--spacing-sm) var(--spacing-md)', textAlign: 'center' }}>
                                                        <div style={{ display: 'flex', gap: '4px', justifyContent: 'center' }}>
                                                            <button
                                                                ...
                                                                className="btn btn-ghost"
                                                                title={card.isLocked ? 'Lock Card' : 'Unlock Card'}
                                                                style={{ color: card.isLocked ? 'var(--color-accent-orange, #f59e0b)' : 'var(--color-accent-green)', padding: '4px' }}
                                                                disabled={actionLoading}
                                                            >
                                                                {card.isLocked ? <Lock size={16} /> : <Unlock size={16} />}
                                                            </button>
                                                            <button
