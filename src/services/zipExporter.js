 ...
 */
export const exportAccountsAsZip = async (accounts, onProgress) => {
  const zip = new JSZip();
  ...
  for (let i = 0; i < accounts.length; i += batchSize) {
    ...
    await Promise.all(
      batch.map(async (account, index) => {
        const pdf = await generateAccountPDF(account);
 ...
 */
export const downloadAccountsZip = async (accounts, onProgress) => {
  const zipBlob = await exportAccountsAsZip(accounts, onProgress);
